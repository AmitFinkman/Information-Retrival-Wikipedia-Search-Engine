...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes, bucket_name):
        ...
        for loc in locs:
            f_name = loc[0]
            offset = loc[1]
            fs = gcsfs.GCSFileSystem(project='homework3-370213')
...
class InvertedIndex:
    ...
    def posting_lists_iter(self, bucket_name):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(self.posting_locs[0], self.df[w] * TUPLE_SIZE, bucket_name)
                posting_list = []
    ...
    def get_posting_list_term(self, w, threshold=2000):
        TUPLE_SIZE = 6
        with closing(MultiFileReader()) as reader:
            locs = self.posting_locs[w]
            b = reader.read(locs, self.df[w] * TUPLE_SIZE, self.bucket_name)
            posting_list = []
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
                tf = int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
